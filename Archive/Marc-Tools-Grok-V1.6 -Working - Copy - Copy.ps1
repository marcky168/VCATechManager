...
"@
    ...
    function Woofware-ErrorsCheckByUser {
        ...
        # Debug: Output selected username and total results
        Write-Host "Debug: Selected Username: $Username" -ForegroundColor Cyan
        Write-Host "Debug: Total errors collected: $($results.Count)" -ForegroundColor Cyan

        # Debug: Show a sample message to inspect format
        if ($results) {
            Write-Host "Debug: Sample error message (first 500 chars): $($results[0].Message.Substring(0, [Math]::Min(500, $results[0].Message.Length)))" -ForegroundColor Cyan
        }

        # Now filter by username in the main function
        $filteredErrors = $results | Where-Object { $_.Message -ilike "*VCAANTECH\$Username*" }

        # Debug: Output filtered errors count
        Write-Host "Debug: Errors after filtering for user '$Username': $($filteredErrors.Count)" -ForegroundColor Cyan
