...
# Set version
$version = "1.7"  # Updated for enhancements: parameter validation, expanded error handling, credential checks, and empty result checks
...
if (Test-Path $credPathAD) {
    try {
        ...
        Write-Log "Failed to load saved AD credentials: $($_.Exception.Message)"
    }
} else {
    $ADCredential = $null
}
...
function Get-CachedServers {
    ...
    try {
        $servers = Get-ADComputer -Filter "Name -like '$SiteAU-ns*' -and Name -notlike '*CNF:*' -and OperatingSystem -like '*Server*'" -Server "vcaantech.com" -Credential $ADCredential -ErrorAction Stop | Select-Object -ExpandProperty Name | Sort-Object Name
...
try {
    ...
    function Get-VCAServers {
        ...
        $SiteAU = Convert-VcaAu -AU $AU -Suffix ''
        try {
            $adServers = Get-ADComputer -Filter "Name -like '$SiteAU-ns*' -and Name -notlike '*CNF:*' -and OperatingSystem -like '*Server*'" -Server "vcaantech.com" -Credential $ADCredential -ErrorAction Stop | Select-Object -ExpandProperty Name | Sort-Object Name
