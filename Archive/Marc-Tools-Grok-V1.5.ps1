...
$privateFiles = Get-ChildItem -Path "$PSScriptRoot\Private" -Filter *.ps1
...
while (-not $exitScript) {
    ...
    while ($menuActive) {
        Write-Host ""
        Write-Host "[$(Get-Date -Format "MM/dd/yyyy h:mm tt")][ AU $AU ?]: " -NoNewline -ForegroundColor Yellow  # Changed color to Yellow and added space
