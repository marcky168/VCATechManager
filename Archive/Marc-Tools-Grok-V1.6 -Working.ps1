...
function Write-ConditionalLog {
    ...
    if ($verboseLogging) {
        Add-Content -Path $logPath -Value "[$(Get-Date -Format "MM/dd/yyyy h:mm tt")] $Message"
        Write-Host "Log written: $Message" -ForegroundColor Cyan  # Debug: Confirm log write
    }
}
...
function Get-CachedServers {
    ...
    try {
        $servers = Get-ADComputer -Filter "Name -like '$SiteAU-ns*' -and Name -notlike '*CNF:*' -and OperatingSystem -like '*Server*'" -Properties Name -Credential $ADCredential | Select-Object -ExpandProperty Name | Sort-Object Name
...
try {
    ...
    if (-not $HospitalMaster) {
        ...
        if (Test-Path $hospitalMasterPath) {
            try {
                ...
                Write-Host "Debug: HospitalMaster loaded with $($HospitalMaster.Count) entries from $hospitalMasterPath" -ForegroundColor Cyan
            } catch {
    ...
    function Get-VCAServers {
        ...
        $SiteAU = Convert-VcaAu -AU $AU -Suffix ''
        try {
            $adServers = Get-ADComputer -Filter "Name -like '$SiteAU-ns*' -and Name -notlike '*CNF:*' -and OperatingSystem -like '*Server*'" -Properties Name -Credential $ADCredential | Select-Object -ExpandProperty Name | Sort-Object Name
            if (-not $adServers) {
    ...
    function Abaxis-MacAddressSearch {
        param([string]$AU)
        Write-Host "Debug: Abaxis-MacAddressSearch function called with AU $AU" -ForegroundColor Cyan  # Debug: Confirm function call
       ...
       # Add nslookup for Hxxxx-fuse
        ...
        if ($fuseIpAddresses -and $fuseIpAddresses.Length -gt 0) {
            ...
            Write-Host "`nFuse Device IP (from nslookup on $fuseHostname): " -ForegroundColor Green -NoNewline
                ...
                ScriptBlock  = {
                    ...
                    try {
                        ...
                        Invoke-Command -ComputerName $server -SessionOption $sessionOption -ScriptBlock {
                            ...
                            $allErrors = @()
                            $errors100101102 = Get-WinEvent -FilterHashtable @{logname='Application';Providername='Woofware'; level=2 ;id=100,101,102} -MaxEvents 50 -ErrorAction SilentlyContinue | Select-Object @{n='Server';e={$using:server}}, @{n='ErrorType';e={"100, 101 or 102"}}, TimeCreated, Id, Message, LevelDisplayName
                            if ($errors100101102) { $allErrors += $errors100101102 }
                            $errors102 = Get-WinEvent -FilterHashtable @{logname='Application';Providername='Woofware'; level=2 ;id=102} -MaxEvents 1 -ErrorAction Ignore | Select-Object @{n='Server';e={$using:server}}, @{n='ErrorType';e={"102"}}, TimeCreated, Id, Message, LevelDisplayName
        ...
        try {
            ...
            if (-not $SkipPropertiesDisplay) {
                try {
                    ...
                    $adUser = Get-ADUser -Identity $Username -Properties Name, Title, OfficePhone, Office, Department, EmailAddress, StreetAddress, City, State, PostalCode, SID, Created, extensionAttribute3, PasswordLastSet -Credential $ADCredential -ErrorAction Stop
            ...
            if ($activeSessions) {
                ...
                if ($selectedSession) {
                    ...
                    if ($launchChoice.ToLower() -eq 'v') {
                        ...
                        # Launch RDP Shadow
                        Start-Process "mstsc.exe" -ArgumentList @("/v:$($selectedSession.Computer)", "/shadow:$($selectedSession.SessionId)", "/control")
                        Write-Host "Launching RDP Shadow for $($selectedSession.UserName) on $($selectedSession.Computer)." -ForegroundColor Green
                        Write-Log "Launched RDP Shadow for $($selectedSession.UserName) on $($selectedSession.Computer)"
                    } else {
                        Write-Host "Operation cancelled." -ForegroundColor Yellow
                    }
                }
    ...
    function ADUserManagement {
        ...
        try {
            ...
            $groupMembers = Get-ADGroupMember -Identity $groupName -Credential $Credential -ErrorAction Stop
            $users = $groupMembers | Get-ADUser -Properties Name, SamAccountName, EmailAddress, LockedOut, PasswordExpired, LastLogonDate -Credential $Credential -ErrorAction Stop
            ...
            # Display users in grid for selection
    ...
    # New: Periodic update check (every 12 hours)
    $lastUpdateCheck = $script:LastUpdateCheck
    if (-not $lastUpdateCheck -or (New-TimeSpan -Start $lastUpdateCheck -End (Get-Date)).Hours -ge 12) {
        try {
            Update-HospitalMaster
            $script:LastUpdateCheck = Get-Date
        } catch {
            Write-Host "Error during periodic update check: $($_.Exception.Message)" -ForegroundColor Red
            Write-Log "Periodic update check error: $($_.Exception.Message)"
        }
    }
    ...
    while (-not $exitScript) {
        ...
        try {
            ...
            Write-Host "Failed to query AD group '$groupName' for AU $AU. Error: $($_.Exception.Message)" -ForegroundColor Red
            Write-Log "Failed to query AD group '$groupName' for AU $AU. Error: $($_.Exception.Message)"
            continue
        }
        ...
        while ($menuActive) {
            ...
            switch ($choice) {
                ...
                "8" {
                    ...
                    Write-Host "13. Launch ServiceNow: Opens ServiceNow incident list filtered by AU." -ForegroundColor White
                ...
                default {
                    Write-Host "Invalid choice. Please select 0-14u." -ForegroundColor Red
                }
            }
        }
    }
